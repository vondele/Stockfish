...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const HistoryContainer* hc_p, Square s)
           : pos(p), hc(hc_p) {
...
void MovePicker::score<QUIETS>() {
  ...
  for (auto& m : *this)
      m.value = hc->eval(pos.side_to_move(), pos.moved_piece(m), m);
}
...
void MovePicker::score<EVASIONS>() {
  ...
  for (auto& m : *this)
      if (pos.capture(m))
          ...
          m.value = hc->eval(pos.side_to_move(), m);
}
