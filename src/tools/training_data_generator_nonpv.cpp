...
namespace Stockfish::Tools
{
        ...
        int count)
    {
        ...
        const bool is_frc = Options["UCI_Chess960"];
        for (int i = 0; i < count; ++i)
        {
            if (opening_book != nullptr)
            {
                auto& fen = opening_book->next_fen();
                pos.set(fen, is_frc, &si, &th);
            }
            else
            {
                pos.set(StartFEN, is_frc, &si, &th);
            }
        ...
        uint64_t limit)
    {
        ...
        while (!quit)
        {
            ...
            {
                pos.set_from_packed_sfen(ps.sfen, &si, &th, is_frc);
                pos.state()->rule50 = 0;
    ...
    bool TrainingDataGeneratorNonPv::commit_psv(
        ...
        uint64_t limit)
    {
        const bool is_frc = th.rootPos.is_chess960();
        ...
        for (auto& sfen : sfens)
        {
            ...
            if (is_frc && type_of((Move)sfen.move) == CASTLING)
            {
                continue;
            }
