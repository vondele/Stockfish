...
#undef S
  ...
  Score Evaluation<T>::initiative(Score score) const {
    ...
    int u = ((mg > 0) - (mg < 0)) * std::max(std::min(complexity + 50, 0), -abs(mg) + pos.this_thread()->rootIsDraw ? 2 : 0);
    int v = ((eg > 0) - (eg < 0)) * std::max(complexity, -abs(eg) + pos.this_thread()->rootIsDraw ? 2 : 0);
...
std::string Eval::trace(const Position& pos) {
  ...
  pos.this_thread()->rootIsDraw = false;
