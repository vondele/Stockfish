...
Value scale_evaluation(Value nnue, int optimism, const Position& pos) {
    ...
    // 1. Normalize the raw evaluations to [-1024, 1024]
    int se_norm   = (se * 1024) / (std::abs(se) + 1024);
    int nnue_norm = (nnue * 1024) / (std::abs(nnue) + 1024);

    // 2. Measure "alignment" between se and nnue as a metric of position difficulty.
    // [hard, average, easy] -> [0, 2048, 4096].
    int alignment = (se_norm * nnue_norm) / 512 + 2048;
