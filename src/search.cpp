...
namespace {
  ...
  Value value_draw(int ply, Thread* thisThread) {
    return VALUE_DRAW + (1 + ply / 16) * Value(2 * (thisThread->nodes & 1) - 1);
  }
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        alpha = value_draw(ss->ply, pos.this_thread());
        if (alpha >= beta)
            return alpha;
    }
    ...
    if (!rootNode)
    {
        ...
        if (   Threads.stop.load(std::memory_order_relaxed)
            ...
            return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate(pos)
                                                        : value_draw(ss->ply, pos.this_thread());
    ...
    else if (ss->ttHit)
    {
        ...
        if (eval == VALUE_DRAW)
            eval = value_draw(ss->ply, thisThread);
