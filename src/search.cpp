...
namespace {
  ...
  Value value_draw(Position& pos, Depth depth) {
    if (depth < 4)
        return VALUE_DRAW;
    else 
    {
        Color us = pos.side_to_move();
        Value matDiff = (pos.count<PAWN>( us) - pos.count<PAWN>(~us)) * PawnValueMg
                       + pos.non_pawn_material( us) - pos.non_pawn_material(~us);
        return   VALUE_DRAW
               + Value(matDiff >= 0)
               + Value(2 * (pos.this_thread()->nodes.load(std::memory_order_relaxed) % 2) - 1);
     }
  }
