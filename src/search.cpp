...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    /*
    if (thisThread->nodes % 1024 == 0)
        std::cout << "xxx " << float(thisThread->ttProgress) / (ttProgressResolution * ttProgressWindow) << std::endl;
    */
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || cutNode))
      {
          ...
          if (16 * thisThread->ttProgress > 7 * ttProgressResolution * ttProgressWindow)
             r--;
