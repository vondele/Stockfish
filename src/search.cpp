...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth / ONE_PLY > Limits.depth))
  {
      ...
      if (!Threads.stop)
      {
          completedScore.store(rootMoves[0].score, std::memory_order_relaxed);
          completedDepth.store(rootDepth, std::memory_order_relaxed);
      }
      ...
      if (Limits.use_time_management())
      {
          if (!Threads.stop && !Threads.stopOnPonderhit)
          {
              ...
              const int F[] = { failedLow, bestValue - Threads.previousScore };
              ...
              for (int i : {3, 4, 5})
                  if (lastBestMoveDepth * i < completedDepth && !thinkHard)
                     timeReduction *= 1.3;
              unstablePvFactor *=  std::pow(Threads.previousTimeReduction, 0.51) / timeReduction;
