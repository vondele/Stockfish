...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    tte = TT.probe(posKey, ttHit, ((thisThread->nodes.load(std::memory_order_relaxed) & 0x30) == 0) * (2 * (thisThread->nodes.load(std::memory_order_relaxed) % 2) - 1));
...
bool RootMove::extract_ponder_from_tt(Position& pos) {
    ...
    pos.do_move(pv[0], st);
    TTEntry* tte = TT.probe(pos.key(), ttHit, 0);
