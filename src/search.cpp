...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    Piece movedPiece;
    int moveCount, captureCount, quietCount, equalCount;

    // Step 1. Initialize node
    Thread* thisThread = pos.this_thread();
    inCheck = pos.checkers();
    Color us = pos.side_to_move();
    moveCount = captureCount = quietCount = equalCount = ss->moveCount = 0;
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_MATED_IN_MAX_PLY)
      {
              ...
              && (!pos.advanced_pawn_push(move) || pos.non_pawn_material() >= Value(5000)))
          {
              ...
              int lmrDepth = std::max(newDepth - reduction<PvNode>(improving, depth, moveCount - equalCount / 2), DEPTH_ZERO) / ONE_PLY;
