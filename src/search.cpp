...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
      ...
      moveCountPruning =   depth < 16 * ONE_PLY
                        && moveCount - drawCount / 2 >= FutilityMoveCounts[improving][depth / ONE_PLY];
          ...
          && bestValue > VALUE_MATED_IN_MAX_PLY)
      {
              ...
              && (!pos.advanced_pawn_push(move) || pos.non_pawn_material() >= Value(5000)))
          {
              ...
              int lmrDepth = std::max(newDepth - reduction<PvNode>(improving, depth, moveCount - drawCount / 2), DEPTH_ZERO) / ONE_PLY;
      ...
      {
          Depth r = reduction<PvNode>(improving, depth, moveCount - drawCount / 2);
